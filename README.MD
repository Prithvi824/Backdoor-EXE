...
-   **Modular utilities** – Three focused helpers implemented under `Utilities/`:
    -   `photographer` – Capture full-screen screenshots and return them as shareable URL.
...
-   **Discord notifications** – Send notifications to Discord when a client connects or disconnects.
...
| `take-photo`      | photographer | –         | Capture a photo from the camera and upload it; returns a shareable URL.  |
...
| `download-file`   | file_manager | `url`     | Download file from URL and save it to the current directory.    |
...
>>dir C:\Users<<
>>use file_manager upload-file C:\Users\prith\Desktop\test.txt<<
```
...
### 4.2 Adding the exe to the windows registry

1. Open the registry editor
2. Navigate to `HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Run`
3. Add a new string value with the name `monitor` and the value `"C:\\Path\\To\\monitor.exe"`

#### Using command

```bash
reg add "HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Run" /v monitor /t REG_SZ /d "C:\\Path\\To\\monitor.exe" /f
```

### 4.3 Common CLI commands
...
## 5. Environment Variables (`config.py`)
